 ...
 * 
 * @Title: MsgListAdapter.java 
 * @Prject: VkoCircleS
 * @Package: cn.vko.ring.message.adapter 
 * @Description: TODO
 * @author: JiaRH   
 * @date: 2015-12-1 上午10:44:53 
 * @version: V1.0   
 */
package cn.vko.ring.message.adapter;
...
public class MsgListAdapter extends BaseAdapter {
	...
	public MsgListAdapter(List<MsgItemModel> msgLists, Context context, Activity act, int type, XListView listview) {
		super();
...
//				StartActivityUtil.startActivity(mContext, MsgGroupAuditActivity.class);
	...
	private void dealNotGroupMsgSkip(MsgItemModel item) {
		if (item.getType() == null || item.getMsg().equals("")) return;
...
//				gotoProblemActivity(item);
	...
	private void deal(final MsgItemModel item, final int type) {
		switch (type) {
			case BaseMsgModel.MSG_SYS_ICON:
				...
				if (item.isNew()) {
					viewHolderSys.msgNewIcon.setVisibility(View.VISIBLE);
				} else {
					viewHolderSys.msgNewIcon.setVisibility(View.GONE);
				}
				viewHolderSys.container.setOnClickListener(new OnClickListener() {
					@Override
			...
			case BaseMsgModel.MSG_LINK_TEXT:
				...
				if (!TextUtils.isEmpty(item.getImgUrl())) {
					ImageListener listener = ImageLoader.getImageListener(viewHolder.linkbg,
			...
			case BaseMsgModel.MSG_GROUP_AUDIT:
				...
				viewHolder.msgTime.setText(item.getCtime());
				if (!TextUtils.isEmpty(item.getImgUrl())) {
					ImageListener listener = ImageLoader.getImageListener(viewHolder.msgLeftIcon,
				...
				viewHolder.refuseBtn.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
				...
				viewHolder.msgDetail.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						goToDetail(item, type);
					}
		...
		if (convertView == null) {
			switch (type) {
				case BaseMsgModel.MSG_SYS_ICON:
					...
					convertView = inflater.inflate(R.layout.item_msg_sys_list_view, null);
					viewHolderSys.msgLeftIcon = (ImageView) convertView.findViewById(R.id.left_msg_icon);
				...
				case BaseMsgModel.MSG_GROUP_AUDIT:
					...
					convertView = inflater.inflate(R.layout.item_msg_list_view, parent, false);
					viewHolder.msgTitle = (TextView) convertView.findViewById(R.id.msg_title);
					viewHolder.msgInfo = (TextView) convertView.findViewById(R.id.msg_detail);
					...
					viewHolder.msgLeftIcon = (RoundAngleImageView) convertView.findViewById(R.id.left_msg_image);
	...
	class ViewHolder {
		...
		TextView tvRefuse;
		TextView msgTime;
		RoundAngleImageView msgLeftIcon;
		View lineView;
		Button refuseBtn;
		Button sureBtn;
		TextView msgDetail;

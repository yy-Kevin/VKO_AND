...
public class PhotoGallery extends Gallery {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof PhotoView) {
			...
			float min_distance = this.getWidth() / 4f;
			...
			if ((int) width <= this.getWidth()
					&& (int) height <= getHeight())// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0 || right < this.getWidth()) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < this.getWidth() || left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
						imageView.postTranslate(-distanceX, -distanceY);
					}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof PhotoView) {
				...
				// + getHeight());
				if ((int) width <= this.getWidth()
						&& (int) height <= getHeight())// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
...
//					imageView.postTranslateDur(-top, 200f);
					imageView.postTranslateDur(getHeight()
							- bottom, 200f);
				}
				if (top > 0 && bottom >getHeight()) {
...
//					imageView.postTranslateXDur(-left, 200f);
					imageView.postTranslateXDur(this.getWidth()
							- right, 200f);
				}
				if(left>0 && right>this.getWidth()){
//					imageView.postTranslateXDur(this.getWidth()

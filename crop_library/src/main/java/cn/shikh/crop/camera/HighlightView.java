...
import cn.shikh.crop.util.PaintUtil;
...
class HighlightView {
	...
	Matrix mMatrix;
	public boolean mGuidelines = true;
	...
	private Paint mGuidelinePaint = PaintUtil.newGuidelinePaint();
	private Paint mCornerPaint = PaintUtil.newCornerPaint();
	...
	private float mCornerOffset = 10 / 2;
	private float mCornerLength = 40;
	...
	private void drawCorners(Canvas canvas) {
		...
		// Top left
		canvas.drawLine(left + mCornerOffset, top + mCornerOffset, left
				+ mCornerLength, top + mCornerOffset, mCornerPaint);
		canvas.drawLine(left + mCornerOffset, top, left + mCornerOffset, top
				+ mCornerLength, mCornerPaint);

		// Top right
		canvas.drawLine(right - mCornerLength, top + mCornerOffset, right, top
				+ mCornerOffset, mCornerPaint);
		canvas.drawLine(right - mCornerOffset, top, right - mCornerOffset, top
				+ mCornerLength, mCornerPaint);

		// Bottom left
		canvas.drawLine(left + mCornerOffset, bottom - mCornerLength, left
				+ mCornerOffset, bottom, mCornerPaint);
		canvas.drawLine(left, bottom - mCornerOffset, left + mCornerLength,
				bottom - mCornerOffset, mCornerPaint);

		// Bottom left
		canvas.drawLine(right - mCornerOffset, bottom - mCornerLength, right
				- mCornerOffset, bottom, mCornerPaint);
		canvas.drawLine(right - mCornerLength, bottom - mCornerOffset, right,
				bottom - mCornerOffset, mCornerPaint);
	}
	...
	private void drawZoomDraw(Canvas canvas) {
		...
		mResizeDrawableWidth
				.setBounds(left - widthWidth, yMiddle - widthHeight, left
						+ widthWidth, yMiddle + widthHeight);
		mResizeDrawableWidth.draw(canvas);

		mResizeDrawableWidth.setBounds(right - widthWidth, yMiddle
				- widthHeight, right + widthWidth, yMiddle + widthHeight);
		mResizeDrawableWidth.draw(canvas);

		mResizeDrawableHeight.setBounds(xMiddle - heightWidth, top
				- heightHeight, xMiddle + heightWidth, top + heightHeight);
	...
	public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean circle,
			boolean square, boolean maintainAspectRatio) {
		if (circle) {

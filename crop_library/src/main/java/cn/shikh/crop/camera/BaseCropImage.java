...
import cn.shikh.crop.camera.gallery.IImage;
import cn.shikh.crop.camera.gallery.IImageList;


/**
 * The activity can crop specific region of interest from an image.
 */
public abstract class BaseCropImage extends MonitoredActivity {
	private static final String TAG = "CropImage";
	public static final int DEFAULT_GUIDELINES = 2;
	public static final boolean DEFAULT_FIXED_ASPECT_RATIO = false;
	public static final int DEFAULT_ASPECT_RATIO_X = 1;
	public static final int DEFAULT_ASPECT_RATIO_Y = 1;
	public static int mType = 2;// 1 ���� 2 �ı���
	...
	private boolean mSquareCrop = false;
	...
	public void onCreate(Bundle icicle) {
		...
		// Work-around for devices incapable of using hardware-accelerated
		// clipPath.
		...
		if (extras != null) {
			...
			} else if (extras.getBoolean("squareCrop", false)) {
				mSquareCrop = true;
				mAspectX = 1;
				mAspectY = 1;
				setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
			...
			mOutlineColor = extras.getInt("outlineColor",
					HighlightView.DEFAULT_OUTLINE_COLOR);
			mOutlineCircleColor = extras.getInt("outlineCircleColor",
					HighlightView.DEFAULT_OUTLINE_CIRCLE_COLOR);
		...
		getDisCardView().setOnClickListener(
		...
		getSaveView().setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				onSaveClicked();
			}
		});
	...
	public abstract CropImageView getCropImageView();
	public abstract TextView getDisCardView();
	public abstract TextView getSaveView();
	private void startFaceDetection() {
		...
		Util.startBackgroundJob(this, null,"请稍候..."
				,
				new Runnable() {
					public void run() {
	...
	private void onSaveClicked() {
		...
		if (myExtras != null
			...
			final String msdId = mSetWallpaper ?"没有SD卡..."
					: "保存图片...";
		...
		if (mSaveUri != null) {
			...
			try {
				...
				if (outputStream != null) {
					croppedImage.compress(mOutputFormat, mOutputQuality,
							outputStream);
				}
	...
	Runnable mRunFaceDetection = new Runnable() {
		...
		private void handleFace(FaceDetector.Face f) {
			...
			HighlightView hv = new HighlightView(mImageView, mOutlineColor,
					mOutlineCircleColor);
		...
		public void run() {
			...
			mHandler.post(new Runnable() {
				public void run() {
					...
					if (mNumFaces > 1) {
						Toast t = Toast.makeText(BaseCropImage.this,"multiface caop help",
								Toast.LENGTH_SHORT);
						t.show();
					}
				}
...
class CropImageView extends ImageViewTouchBase {
	...
	public boolean onTouchEvent(MotionEvent event) {
		BaseCropImage cropImage = (BaseCropImage) this.getContext();
			...
			if (cropImage.mWaitingToPick) {
				for (int i = 0; i < mHighlightViews.size(); i++) {
					...
					if (hv.hasFocus()) {
						...
						centerBasedOnHighlightView(hv);
						((BaseCropImage) this.getContext()).mWaitingToPick = false;
						return true;
					}
				}
